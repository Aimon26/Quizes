...
@Composable
fun OverallLeaderBoardScreen(    navController: NavController,){
    ...
    Column(
        ...
        when {
            ...
            state.error != null -> {
                ...
                val leaderboard = state.success ?: emptyList()
...
fun PreviewLeaderboard() {
    val sample = listOf(
        Board(uuid = "User123", score = 95),
        Board(uuid = "User456", score = 90),
        Board(uuid = "User789", score = 85)
    )
    QuizesTheme {
        Column(
            modifier = Modifier
